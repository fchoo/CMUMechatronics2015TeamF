...
cDataArray g_dist    = new cDataArray(200);
cGraph g_graph         = new cGraph(20, 190, 780, 380);
Serial g_serial;
PFont  g_font;
...
void draw()
{
  ...
  strokeWeight(1.5);
  stroke(255, 0, 0);
  g_graph.drawLine(g_dist, 0, 1024);
  stroke(0, 255, 0);
...
void processSerialData()
{
  ...
  while (g_serial.available() < 2);  // Loop until we have a full set of data
  // This reads in one set of data
  {
    ...
    dist = ((int)(inBuf[1]&0xFF) << 8) + ((int)(inBuf[0]&0xFF) << 0);
    // g_serial.readBytes(inBuf);
    ...
    {
      // String tempStr;
      // tempStr = dist + "," + yAccel + "," + zAccel + "," + vRef + "," + xRate + "," + yRate + "\r\n";
      // FileWriter file;
    ...
    /*
    print(dist);  print(" ");   print(yAccel);   print(" ");    print(zAccel);     print(" ");
